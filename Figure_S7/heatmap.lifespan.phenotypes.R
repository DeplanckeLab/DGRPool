...
data.correlation_spearman <- fread("../resources/phenotype_correlation_pearson.tsv", header = T, sep = "\t", data.table = F)
...
# Load pre-computed data
data.p_spearman <- fread("../resources/phenotype_correlation_pearson_pvalue.tsv", header = T, sep = "\t", data.table = F)
...
sig.both <- names(fdr_both)[fdr_both <= 0.25]
...
suppressWarnings(p <- pheatmap(data.heatmap, clustering_method = "ward.D", width = 20, height = 10, legend = T, scale = "none", color = colorRampPalette(c("blue", "white", "red"))(100), na_col="white", border_color ="white", show_colnames = F, show_rownames = T, silent = T))
