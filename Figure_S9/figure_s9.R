...
p_list[[as.character(cutoff)]] <- ggplot(df, aes(x = SignificantVariants)) + 
  geom_histogram(bins = 100, fill = "#6A73B4") +
...
# Plot the histogram
df <- data.frame(Phenotype = names(significant_counts), SignificantVariants = unlist(significant_counts[1,]))
df$SignificantVariants[df$SignificantVariants > 10000] <- 10000
p_list[[as.character(cutoff)]] <- ggplot(df, aes(x = SignificantVariants)) + 
  geom_histogram(bins = 100, fill = "#6A73B4") +
  labs(title = bquote(p <= 1 %*% 10^-3), x = bquote("Nb significant GWAS variants / phenotype at" ~ p <= 1 %*% 10^-3), y = "Frequency") +
  geom_hline(yintercept = -10, color = "black") +
  theme(plot.margin = unit(c(0.5,2,0.5,2), 'lines'), axis.title.x = element_text(size = 8), axis.title.y = element_text(size = 8), axis.text.y = element_text(size = 8), axis.text.x = element_text(size = 8), panel.grid.minor = element_line(color = "lightgrey"), panel.grid.major = element_line(color = "lightgrey"), panel.background = element_blank(), plot.background = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(), axis.line.y = element_line(color = "black"), plot.title = element_text(hjust = 0.5, size = 8)) +
  scale_x_continuous(breaks = seq(0, 10000, by = 2000), labels = c("0", "2k", "4k", "6k", "8k", ">10k")) +
  scale_y_continuous(breaks = seq(0, 800, by = 20), expand = c(0, 0))
...
# Plot the histogram
df <- data.frame(Phenotype = names(significant_counts), SignificantVariants = unlist(significant_counts[1,]))
df$SignificantVariants[df$SignificantVariants > 2000] <- 2000
p_list[[as.character(cutoff)]] <- ggplot(df, aes(x = SignificantVariants)) + 
  geom_histogram(bins = 100, fill = "#6A73B4") +
  labs(title = bquote(p <= 1 %*% 10^-4), x = bquote("Nb significant GWAS variants / phenotype at" ~ p <= 1 %*% 10^-4), y = "Frequency") +
  geom_hline(yintercept = -10, color = "black") +
  theme(plot.margin = unit(c(0.5,2,0.5,2), 'lines'), axis.title.x = element_text(size = 8), axis.title.y = element_text(size = 8), axis.text.y = element_text(size = 8), axis.text.x = element_text(size = 8), panel.grid.minor = element_line(color = "lightgrey"), panel.grid.major = element_line(color = "lightgrey"), panel.background = element_blank(), plot.background = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(), axis.line.y = element_line(color = "black"), plot.title = element_text(hjust = 0.5, size = 8)) +
  scale_x_continuous(breaks = seq(0, 2000, by = 500), labels = c("0", "500", "1000", "1500", ">2000")) +
  scale_y_continuous(breaks = seq(0, 800, by = 20), expand = c(0, 0))
...
# Plot the histogram
df <- data.frame(Phenotype = names(significant_counts), SignificantVariants = unlist(significant_counts[1,]))
message("1 x 10^-5 threshold yielded ", round(mean(df$SignificantVariants)), " significant hits on average (median = ", median(df$SignificantVariants),")")
...
# Plot the histogram
df <- data.frame(Phenotype = names(significant_counts), SignificantVariants = unlist(significant_counts[1,]))
message("Bonferroni threshold = ", formatC(cutoff, format = "e", digits = 2), " yielded ", round(mean(df$SignificantVariants)), " significant hits on average (median = ", median(df$SignificantVariants),")")
