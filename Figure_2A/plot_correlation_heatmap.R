...
pheatmap(data.heatmap[row.names(curated.studies), row.names(curated.studies)], legend = F, annotation_legend = F, gaps_col = xpos_curated[unique(curated.studies$Study)], gaps_row=xpos_curated[unique(curated.studies$Study)], annotation_col = data.annot_studies, annotation_names_col = F, annotation_row = data.annot_studies, annotation_names_row = F, color = colorRampPalette(c("blue", "white", "red"))(100), show_rownames = F, show_colnames = F, cluster_rows=F, cluster_cols=F, na_col="white", border_color ="white")
dev.off()
