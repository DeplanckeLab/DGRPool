...
## Working directory
setwd("DGRPool/") # Root directory
dir.create("GWAS", mode="0750", showWarnings = F)
dir.create("GWAS_full_results", mode="0750", showWarnings = F)
...
## Load annotation
annotation.dr <- fread ("dgrp.fb557.annot.txt", header = FALSE, sep = "\t")
...
for(sex in c("F", "M", "NA")) {
	...
	for(i in 2:ncol(phenotype_all)){ # First column is DGRP
		...
		phenotype_name <- paste0(colnames(phenotype_all)[i], "_", sex) # It's not by name, but by ID now
		# Fix weird names
...
message("Total time = ", toReadableTime(as.numeric(Sys.time() - start_time, unit = "secs"))) # Total time = 1h 48mn 31s
