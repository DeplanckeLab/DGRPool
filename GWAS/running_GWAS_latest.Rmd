---
title: "DGRPool - Global GWAS script for all phenotypes"
...
knitr::opts_knit$set(root.dir = "/data/gardeux/DGRPool/")
getwd()
```
...
```{r}
# Parameters
input_covariate_file <- "/data/gardeux/DGRPool/dgrp.cov.tsv"
intput_bed_file <- "/data/gardeux/DGRPool/dgrp2.filtered.fastlmm"
#input_pheno_object <- "data.all_pheno_21_03_23_filtered.rds"
input_pheno_object <- "data.all_pheno_15_07_24_filtered.rds"
input_annotation_file <- "/data/genome/drosophila_melanogaster/dm3/dgrp.fb557.annot.txt"
output_gwas_directory <- "/data/gardeux/DGRPool/latest/GWAS"
gwas_options <- "--glm hide-covar --quantile-normalize --variance-standardize --geno 0.2 --maf 0.01"

input_gene_enrichment_script <- "/data/gardeux/DGRPool/run_gene_enrichment.py"
input_flybase_phenotype_file <- "/data/gardeux/DGRPool/flybase_phenotyping.txt"
input_go_file <- "/data/gardeux/DGRPool/flybase_go.txt"
input_ncsu_annot_file <- "/data/gardeux/DGRPool/dgrp.fb557.annot.background.genes.txt"
input_filtering_pval <- 0.001

python <- "python3.12"
nthreads <- 12

dir.create(output_gwas_directory, recursive = T, mode="0750", showWarnings = F)
```
...
cat(bold("stringr"), "version", as.character(packageVersion("stringr")), "\n")
cat(bold("ramwas"), "version", as.character(packageVersion("ramwas")), "\n")
cat(bold("car"), "version", as.character(packageVersion("car")), "\n")
cat(bold("rstatix"), "version", as.character(packageVersion("rstatix")), "\n")
cat(bold("Cairo"), "version", as.character(packageVersion("Cairo")), "\n")
```

## Functions

```{r}
# Function for displaying human-readable running time
...
```{r}
data.all_pheno <- readRDS(file = input_pheno_object)

# Updating DGRP names to match the genotyping data
...
dgrp.cov <- fread (input_covariate_file, header = T, sep = "\t")
...
## Load annotation
```{r}
annotation.dr <- fread (input_annotation_file, header = FALSE, sep = "\t")
...
```{r}
shapiro_all <- data.frame(statistic = -1, p.value = -1)
...
for(sex in c("F", "M", "NA")) {
	...
	for(i in 2:ncol(phenotype_all)){ # First column is DGRP
	  ...
	  ## Extracting phenotype, removing NAs
		...
		message("Processing ", phenotype_name, " ...")
		...
		fwrite(x = format(plink.file_content, nsmall = 1), file = paste0(output_gwas_directory, "/", phenotype_name, ".pheno.all.tsv"), sep = "\t", quote = F, row.names = F, col.names = T, scipen=50)
		
		## Restrict to genotyped samples
		plink.file_content <- subset(plink.file_content, IID %in% dgrp_lines_genotyped)
		fwrite(x = format(plink.file_content, nsmall = 1), file = paste0(output_gwas_directory, "/", phenotype_name, ".pheno.plink2.tsv"), sep = "\t", quote = F, row.names = F, col.names = T, scipen=50)
		...
		fwrite(x = format(dgrp2.file_content, nsmall = 1), file = paste0(output_gwas_directory, "/", phenotype_name, ".pheno.dgrp2.csv"), sep = ",", quote = F, row.names = F, col.names = F, scipen=50)
		...
		if(!is_categorical){
			...
			if(length(list_values) == 0){
				...
				## Extract study_id, phenotype id and sex from the phenotype_name
				matches <- str_match(phenotype_name, "S(\\d+)_([^_]+)_([^_]+)")
				fstudy <- matches[2]
				fphenotype <- matches[3]
				fsex <- matches[4]

				## Covariate analysis (Anova)
				sink(paste0(output_gwas_directory, "/", phenotype_name, ".cov.anova.txt"))
				...
				if(length(covars) > 0){
					...
					print(summary(fit))
				} else message("ERROR in processing phenotype ", phenotype_name, " [i = ", i, "]: No invariant covariates??")
				...
				tryCatch( { 
				  ...
				  # Add to big table
				  shapiro_all[phenotype_name, c("statistic", "p.value")] <- shapiro[,c("statistic", "p.value")]
				...
				if(length(covars) + 2 >= nrow(plink.file_content)){
					...
					tryCatch(
					{
						std.out <<- system(paste0("plink2 --threads ", nthreads, " ", gwas_options, " --covar ", input_covariate_file, " --bfile ", intput_bed_file," --pheno ", output_gwas_directory, "/", phenotype_name, ".pheno.plink2.tsv --out ", output_gwas_directory, "/"), intern = T, ignore.stderr = T, ignore.stdout = F)
					},
					warning=function(cond) {
						...
						else std.out <<- system(paste0("plink2 --threads ", nthreads, " ", gwas_options, " --covar ", input_covariate_file, " --bfile ", intput_bed_file," --pheno ", output_gwas_directory, "/", phenotype_name, ".pheno.plink2.tsv --out ", output_gwas_directory, "/"), intern = T, ignore.stderr = T, ignore.stdout = F)
					})
					
					# Check output of PLINK2
					if(is.null(std.out)){
						message("ERROR in processing phenotype ", phenotype_name, " [i = ", i, "]: No output?")
					} else {
						## Reading Output
						plink_results <- NA
						col6 <- "BETA"
						file_suffix <- ".glm.linear"
						if(file.exists(paste0(output_gwas_directory, "/.", phenotype_name, ".glm.linear"))){
							# Numeric / continuous
							plink_results <<- fread(paste0(output_gwas_directory, "/.", phenotype_name, ".glm.linear"), data.table = F, nThread = nthreads, showProgress = F)
							file.remove(paste0(output_gwas_directory, "/.", phenotype_name, ".glm.linear"))
						} else if(file.exists(paste0(output_gwas_directory, "/.", phenotype_name, ".glm.logistic.hybrid"))){
							# Categorical
						  plink_results <<- fread(paste0(output_gwas_directory, "/.", phenotype_name, ".glm.logistic.hybrid"), data.table = F, nThread = nthreads, showProgress = F)
						  file.remove(paste0(output_gwas_directory, "/.", phenotype_name, ".glm.logistic.hybrid"))
							col6 <<- "OR"
							file_suffix <<- ".glm.logistic.hybrid"
						} else stop("ERROR [Cannot find any output file?]")
						...
						png(paste0(output_gwas_directory, "/", phenotype_name, ".qqplot.png"), height = 500, width = 500)
						...
						#pdf(paste0(output_gwas_directory, "/", phenotype_name, ".qqplot.pdf"), height = 7, width = 7)
						#gap::qqunif(plink_results$P) # Slow
						#qq(plink_results$P) # Slower
						CairoPDF(paste0(output_gwas_directory, "/", phenotype_name, ".qqplot.pdf"), height = 7, width = 7)
						qqPlotFast(plink_results$P, ci.level = NULL, col = "black", makelegend = F, lwd = 1, newplot = T)
						dev.off()
						...
						png(paste0(output_gwas_directory, "/", phenotype_name, ".manhattan.png"), height = 500, width = 500)
						...
						# SVG is way too big. So let's go PDF
						# I recompute the plot... I know it's not optimal but I don't know how to do it else
						#pdf(paste0(output_gwas_directory, "/", phenotype_name, ".manhattan.pdf"), height = 7, width = 7)
						CairoPDF(paste0(output_gwas_directory, "/", phenotype_name, ".manhattan.pdf"), height = 7, width = 7)
						manPlotFast(man = m, lwd = 1, colorSet = c("black", "darkgrey"))
						#manhattan(plink_results, chr="#CHROM", bp = "POS", snp = "ID", chrlabs = c("2L", "2R", "3L", "3R", "X", "4"), suggestiveline = F, genomewideline = F, annotateTop = T, annotatePval = T)
						dev.off()
						...
						fwrite(x = plink_results, file = paste0(output_gwas_directory, "/", phenotype_name, file_suffix, ".fdr.annot.tsv"), sep = "\t", quote = F, row.names = F, col.names = T)
						system(paste0("gzip -f ", output_gwas_directory, "/", phenotype_name, file_suffix, ".fdr.annot.tsv"))
						
						# Filtering
						plink_results <- subset(plink_results, P <= input_filtering_pval)
						fwrite(x = plink_results, file = paste0(output_gwas_directory, "/", phenotype_name, file_suffix, ".top_", input_filtering_pval, ".annot.tsv"), sep = "\t", quote = F, row.names = F, col.names = T)
						system(paste0("gzip -f ", output_gwas_directory, "/", phenotype_name, file_suffix, ".top_", input_filtering_pval, ".annot.tsv"))
						...
						message("Processing ", phenotype_name, " [SUCCESS]")
					} # !There is an output file in PLINK
				} # !phenotype can be run (PLINK loop)
			} # !phenotype is not malformed
		} # !is_categorical
		## Progression bar
		cnt <- cnt + 1
		setTxtProgressBar(pb, cnt)
	} # !For loop on phenotypes
} # !For loop on sex (F, M, NA)
message("Total time = ", toReadableTime(as.numeric(Sys.time() - start_time, unit = "secs"))) # Total time = ~14h
```

```{r}
shapiro_all <- shapiro_all[-1, ]
fwrite(x = shapiro_all, file = paste0(output_gwas_directory, "/shapiro.all.tsv"), sep = "\t", row.names = T, col.names = T, quote = F)
```
