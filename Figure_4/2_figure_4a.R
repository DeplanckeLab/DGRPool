...
p_list[[as.character(cutoff)]] <- ggplot(df, aes(x = SignificantVariants)) + 
  ...
  theme(plot.margin = unit(c(0,0.2,0,1), 'lines'), axis.title.x = element_text(size = 8), axis.title.y = element_text(size = 8), axis.text.y = element_text(size = 8), axis.text.x = element_text(size = 8), panel.grid.minor = element_line(color = "lightgrey"), panel.grid.major = element_line(color = "lightgrey"), panel.background = element_blank(), plot.background = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(), axis.line.y = element_line(color = "black"), plot.title = element_text(hjust = 0.5, size = 8)) +
...
p_list[[as.character(cutoff)]] <- ggplot(df, aes(x = SignificantVariants)) + 
  ...
  scale_y_continuous(breaks = seq(0, 800, by = 100), expand = c(0, 0))
p_list[[as.character(cutoff)]]

p <- ggarrange(plotlist = p_list, ncol = 2, nrow = 4)
ggsave(plot = p, filename = "FigureS9.pdf", width = 7, height = 10)
